...
- **[05 Mag → 06 Mag aggiornato]** **Stripe Checkout idempotency con claim atomico, non con UPDATE post-create** (BUG-052 v1 → v2 da audit Codex). Versione v1 (UPDATE dopo `stripe.checkout.sessions.create`) NON e' atomica: due richieste concorrenti possono leggere entrambe `stripe_session_id = null`, creare entrambe una session su Stripe, fare entrambe l'UPDATE, e una sovrascrive l'altra → orphan session pagabile. **Pattern atomico v2**: usare la stessa colonna `stripe_session_id` come token di lock prima di chiamare Stripe. Step: (1) se SELECT trova session id reale (non `claim:*`), retrieve+riuso; (2) UPDATE `stripe_session_id = 'claim:<random>'` con `.is('stripe_session_id', null).eq('status', 'pending').select('id').maybeSingle()` — solo il PRIMO worker riceve righe (claim vinto); (3) i worker che ricevono 0 righe rileggono il booking: se trovano `claim:*` ritornano 409 `in_progress` (retry client), se trovano session id reale la riusano; (4) il worker claimer chiama `stripe.checkout.sessions.create()`; in caso di errore rilascia il claim con UPDATE filtrato su `claimToken`; (5) success → UPDATE finale sostituisce il token con la session id reale, filtrato su `claimToken` per non sovrascrivere claim arrivati nel frattempo. Pattern generalizzabile a qualsiasi external API call che richiede idempotency su DB row: usare la stessa colonna come token-then-final-id, condizioni `.is(col, null)` per il claim, `.eq(col, claimToken)` per persist e rilascio.
- **[05 Mag]** **Mount WSL su Windows tronca file casualmente durante write multipli rapidi** (sintomo critico). `Write` e `Edit` tool a volte salvano file troncati a metà espressione. `node --check` puo' passare (sintassi bilanciata per caso) ma `next lint` o `npm run build` falliscono con `Parsing error: Unexpected token`. **Pattern di rilevamento**: se vedi parsing error inattesi su file che dovevano essere puliti, prima di assumere bug nel codice fai `wc -l file` + `tail -3 file` per controllare se finisce a metà parola/espressione. **Workaround**: per file critici scrivi via `cat > file << 'EOF' ... EOF` da bash WSL — il pipe atomico evita il problema. La cache ESLint (`.next/cache/eslint/.cache_*`) sul mount Windows non e' rimovibile via `rm` (EPERM permessi mount), ma si svuota da sola al prossimo build successful.
- **[05 Mag]** **Dark mode: strategia centralizzata in globals.css invece di `dark:` sparse**. Tailwind `darkMode: 'class'` + classe `.dark` su `<html>`. Le regole `html.dark .bg-white { ... }`, `html.dark .text-stone-900 { ... }`, etc. sono raggruppate in `app/globals.css` invece di aggiungere `dark:bg-stone-800 dark:text-stone-100` su ogni elemento JSX. Vantaggio: 0 modifiche ai componenti per coprire le classi standard. Svantaggio: il redesign incrementale che introduce gradient (`bg-gradient-to-br`), opacity (`bg-amber-50/60`), bordi-opacity (`border-amber-200/70`) e badge nuovi (`bg-amber-100`) richiede aggiornamento manuale di `globals.css`. **Checklist post-redesign**: dopo aver introdotto classi nuove di Tailwind, sempre verificare che esista la regola `html.dark .<classe>` corrispondente e adattare al colore warm del progetto.
- **[05 Mag]** **Template literal grandi con CSS inline confondono il parser ESLint** (`lib/email-templates.js`). Babel preset di `next lint` perde il conto delle graffe `{`/`}` quando un template literal ha CSS embedded con tante regole + apostrofi italiani in stringhe lunghe. `node --check` accetta. Workaround: aggiungere il file agli `ignorePatterns` di `.eslintrc.json`. Trade-off: il file resta non lintato, ma e' template-only (no logica). Soluzione pulita: estrarre il CSS in costante separata oppure file `.css` esterno + `fs.readFileSync` al boot.
