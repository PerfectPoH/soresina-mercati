...
- **[25 Aprile 2026] Vercel preview deploy → Auth Protection di default:** I preview Vercel su account Hobby/Free hanno `ssoProtection` attivo per default → tutte le richieste non autenticate ricevono 401. Conseguenza: Stripe (e qualsiasi webhook esterno: GitHub, Twilio, Resend) NON può invocare endpoint sui preview. Sintomo classico: il pagamento su Stripe va a buon fine ma il booking resta `pending` perché il webhook non è mai arrivato. Fix: `PATCH /v9/projects/{id}` con `ssoProtection: null`, oppure (più sicuro) configurare "Deployment Protection Exceptions" che esclude solo il branch alias staging.
- **[25 Aprile 2026] `NEXT_PUBLIC_*` env distinte per scope:** Su Vercel, una env var con `target: ['production', 'preview']` usa lo stesso valore in entrambi gli ambienti. Se prod e staging hanno URL diversi (caso tipico), va creato un record per scope. Il sintomo è che il redirect post-pagamento (success_url Stripe, magic link Supabase, ecc.) finisce sul dominio sbagliato → 404 (perché il booking esiste solo nel DB dell'altro ambiente).
